...
export function PollingStationsSettings() {
  ...
  const pollingStationFilterFunction = (station: any, filters: Record<string, string>, searchTerm: string): boolean => {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configuration et gestion des bureaux de vote</CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            <CardTitle>Gestion des bureaux de vote</CardTitle>
            <CardDescription>Configuration et gestion des bureaux de vote ({pollingStations.length} bureaux)</CardDescription>
          </div>
                  ...
                  <tr key={station.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        ...
                        {station.department && <div className="text-gray-600">{station.department}</div>}
                        {station.commune && <div className="text-gray-500">{station.commune}</div>}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        station.status === 'active' ? 'bg-green-100 text-green-800' :
                        station.status === 'closed' ? 'bg-red-100 text-red-800' :
                        station.status === 'results_submitted' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        <div>Inscrits: <span className="font-medium text-blue-600">{station.registered_voters}</span></div>
                        <div>Votants: <span className="font-medium text-green-600">{station.votes_submitted}</span></div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {station.turnout_rate?.toFixed(1) || 0}%
                      </div>
                    </td>
...
function PollingStationFormModal({ station, onClose }: { station: any | null; onClose: () => void }) {
